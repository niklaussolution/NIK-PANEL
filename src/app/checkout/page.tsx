...
import { Server, Check, ChevronDown, Tag, X as XIcon, ArrowLeft, Loader2 } from "lucide-react";
...
import { generateOrderId, createUPIOrder } from "@/lib/orders";
import toast from "react-hot-toast";

// ── Stack dropdown ────────────────────────────────────────────────────────────
const STACKS = [
  { value: "CyberPanel", label: "CyberPanel", desc: "Web hosting control panel", logo: "/assets/icons/cyberpanel.svg", bg: "#0d6b3f" },
  { value: "Docker",     label: "Docker",     desc: "Container runtime",         logo: "/assets/icons/docker.svg",     bg: "#f0f8ff" },
  { value: "Ubuntu",     label: "Ubuntu",     desc: "Clean Ubuntu 22.04 LTS",   logo: "/assets/icons/ubuntu.svg",     bg: "#fff3ee" },
];

function StackLogo({ logo, bg, label }: { logo: string; bg: string; label: string }) {
  return (
    <span className="w-8 h-8 rounded-[8px] flex items-center justify-center shrink-0 overflow-hidden" style={{ backgroundColor: bg }}>
      <Image src={logo} alt={label} width={22} height={22} className="object-contain" />
    </span>
  );
}

function StackDropdown({ value, onChange, error }: { value: string; onChange: (v: string) => void; error?: string }) {
    ...
    <div>
      ...
      <div ref={ref} className="relative">
        <button type="button" onClick={() => setOpen((o) => !o)}
          className={`w-full flex items-center justify-between gap-3 rounded-[10px] border bg-white px-3.5 py-2.5 text-sm text-gray-900 transition-all duration-200 ${open ? "border-[#FF6B00] ring-2 ring-[#FF6B00]/20" : error ? "border-red-400" : "border-gray-200 hover:border-gray-300"}`}>
        ...
        {open && (
          <div className="absolute z-20 mt-1.5 w-full rounded-[12px] border border-gray-100 bg-white shadow-lg overflow-hidden">
            {STACKS.map((stack) => (
              <button key={stack.value} type="button" onClick={() => { onChange(stack.value); setOpen(false); }}
                className={`w-full flex items-center gap-3 px-3.5 py-3 text-left text-sm transition-colors duration-150 ${stack.value === value ? "bg-orange-50" : "text-gray-700 hover:bg-gray-50"}`}>
                <StackLogo logo={stack.logo} bg={stack.bg} label={stack.label} />
                <span className="flex flex-col flex-1">
                  <span className={`font-semibold leading-tight ${stack.value === value ? "text-[#FF6B00]" : "text-gray-900"}`}>{stack.label}</span>
                  <span className="text-xs text-gray-400">{stack.desc}</span>
                </span>
                {stack.value === value && <Check className="w-4 h-4 text-[#FF6B00] shrink-0" />}
              </button>
            ))}
          </div>
        )}
...
function CheckoutContent() {
  ...
  const handleProceed = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/checkout/upi?order=${orderId}`);
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-gray-50 pt-8 pb-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/plans" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-[#FF6B00] transition-colors group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          {/* Form */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }} className="lg:col-span-3">
            <div className="bg-white rounded-[20px] border border-gray-100 shadow-card p-8">
              ...
              <p className="text-sm text-gray-400 mb-6">Complete your details — you will pay via UPI on the next step.</p>
              ...
              <form onSubmit={handleProceed} className="space-y-4">
                ...
                <StackDropdown value={form.os} onChange={(v) => setForm({ ...form, os: v })} error={errors.os} />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input type="text" placeholder="Enter coupon code" value={couponInput} disabled={!!couponApplied}
                    ...
                    {couponApplied ? (
                      ...
                      <button type="button" onClick={handleApplyCoupon} disabled={!couponInput.trim() || couponLoading}
                  ...
                  {couponApplied && (
                    <p className="mt-1 text-xs text-green-600 font-medium">
                      ✓ {couponApplied.discount}% discount applied — saving ₹{((plan.price) - discountedPrice).toLocaleString("en-IN")}/month
                    </p>
                  )}
                </div>
                ...
                {/* UPI notice */}
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-[12px] flex items-start gap-3">
                  <span className="text-xl mt-0.5">📱</span>
                  <div>
                    <p className="text-sm font-semibold text-gray-800">Pay via UPI</p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      On the next step, scan a QR code or use our UPI ID to pay ₹{discountedPrice.toLocaleString("en-IN")}. Then submit your UTR number for instant verification.
                    </p>
                  </div>
                </div>

                <Button type="submit" size="lg" loading={loading} className="w-full mt-2">
                  {loading ? "Creating Order…" : `Proceed to Pay — ₹${discountedPrice.toLocaleString("en-IN")}`}
                </Button>
              </form>
            </div>
          </motion.div>
          ...
          {/* Order Summary */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="lg:col-span-2">
            <div className="bg-white rounded-[20px] border border-gray-100 shadow-card p-6 sticky top-8">
              ...
              <div className="bg-gray-50 rounded-[12px] p-4 mb-5">
                <div className="flex items-start justify-between gap-2 mb-4">
                  ...
                  {plan.popular && (
                    <span className="text-xs bg-orange-100 text-[#FF6B00] font-medium px-2 py-0.5 rounded-full shrink-0">Popular</span>
                  )}
                </div>
                {[
              ...
              <div className="space-y-2 mb-5">
                {["AlmaLinux 9", form.os, "Full Root Access", "KVM Virtualization"].map((item) => (
                  <div key={item} className="flex items-center gap-2 text-sm text-gray-600">
                    <div className="w-4 h-4 bg-orange-50 rounded-full flex items-center justify-center shrink-0">
                ...
                {couponApplied && (
                  <>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-green-600 font-semibold bg-green-50 px-2 py-0.5 rounded-full">-{couponApplied.discount}%</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-green-600 flex items-center gap-1"><Tag className="w-3 h-3" /> {couponApplied.code}</span>
                      <span className="text-xs text-green-600 font-semibold">-₹{(plan.price - discountedPrice).toLocaleString("en-IN")}</span>
                    </div>
                  </>
                )}
                <div className="flex justify-between items-center">
              ...
              <div className="mt-4 p-3 bg-orange-50 border border-orange-100 rounded-[10px] flex items-center gap-2">
                <span className="text-base">📱</span>
                <span className="text-xs text-orange-700 font-medium">Payment via UPI — instant verification</span>
              </div>
              ...
              <div className="mt-3 p-3 bg-green-50 rounded-[10px] flex items-center gap-2">
                ...
                <span className="text-xs text-green-700 font-medium">Provisioning after payment verified</span>
              </div>
            </div>
          </motion.div>
        </div>
