...
import { ShoppingCart, Search, RefreshCw, DollarSign, Trash2, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onCancel}>
      ...
      >
        ...
        <h3 className="text-base font-bold text-center text-gray-900 mb-1">Delete Order?</h3>
        <p className="text-sm text-gray-500 text-center mb-6">
          This will permanently remove the order for <strong>{name}</strong>. This cannot be undone.
        </p>
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl">
      <AnimatePresence>
        {deleteTarget && (
          <DeleteModal
            name={deleteTarget.customerName || deleteTarget.customerEmail || "this order"}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            ...
            <div className={`w-9 h-9 ${s.bg} rounded-full flex items-center justify-center`}>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50">
                  {["Customer", "Plan", "Hostname", "Amount", "Status", "Payment ID", "Date", ""].map((h, i) => (
                    <th key={i} className="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
